...
import { SIYA_VARIANT_MODEL } from '../siya-agent-models'
...
describe('editor agent', () => {
  ...
  describe('default editor definition', () => {
    ...
    test('uses coding model by default', () => {
      expect(editor.model).toBe(SIYA_VARIANT_MODEL.opus)
    })
  ...
  describe('instructions prompt', () => {
    ...
    test('contains Siya_tool_call format', () => {
      expect(editor.instructionsPrompt).toContain('<Siya_tool_call>')
      expect(editor.instructionsPrompt).toContain('</Siya_tool_call>')
    })
