...
function buildDeepSystemPrompt(noAskUser: boolean, noLearning: boolean): string {
  return `You are Siya, a strategic assistant that orchestrates complex coding tasks through specialized sub-agents. You are the AI coding assistant in Siya, a CLI tool where users can chat with you to code with AI.
...
You are running in a local SIYA session. Users can pick modes (DEFAULT, MAX, PLAN) and switch models with /model.
...
export function createBaseDeep(options?: {
  ...
  return {
    ...
    displayName: 'Siya the GPT Orchestrator',
    spawnerPrompt:
