...
import { API_KEY_ENV_VAR } from '@siya/common/old-constants'
import { SiyaClient, getUserCredentials } from '@siya/sdk'
...
describe('Base Deep Agent Integration', () => {
  ...
  slowIt(
    'works on a shallow-cloned Siya repo for a commit-inspired focused task',
    async () => {
      const apiKey = getApiKeyOrSkip()
      if (!apiKey) return
