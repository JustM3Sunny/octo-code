...
import type { FeedbackCategory } from '@siya/common/constants/feedback'
import {
  useCallback,
  useEffect,
  useLayoutEffect,
...
import { getDisplayModelForMode } from './utils/model-settings'
import {
  formatModelSelectionMessage,
  persistModelSelection,
} from './utils/model-selection'
import { resolveNamedTheme } from './utils/named-themes'
...
export const Chat = ({
  ...
  const currentSelectedModelId = useMemo(
    () => getDisplayModelForMode(agentMode, selectedModel),
    [agentMode, selectedModel],
  )
  ...
  useEffect(() => {
    if (!slashContext.active && !modelPickerContext.active && !themePickerContext.active) {
    ...
    slashContext.active,
    slashContext.query,
    modelPickerContext.active,
    modelPickerContext.query,
    themePickerContext.active,
    themePickerContext.query,
    setSlashSelectedIndex,
  ])

  useEffect(() => {
    if (modelPickerContext.active && modelMatches.length > 0) {
      if (slashSelectedIndex >= modelMatches.length) {
        setSlashSelectedIndex(modelMatches.length - 1)
      }
    } else if (themePickerContext.active && themeMatches.length > 0) {
      if (slashSelectedIndex >= themeMatches.length) {
        setSlashSelectedIndex(themeMatches.length - 1)
      }
    } else if (slashMatches.length > 0 && slashSelectedIndex >= slashMatches.length) {
  ...
  const isModelPickerActive = modelPickerContext.active
  const isThemePickerActive = themePickerContext.active
  const isSpecialPickerActive = isModelPickerActive || isThemePickerActive
  const activeSlashSuggestionItems = isModelPickerActive
    ? modelSuggestionItems
    : isThemePickerActive
      ? themeSuggestionItems
      : slashSuggestionItems
  const activeSlashMatchCount = isModelPickerActive
    ? modelMatches.length
    : isThemePickerActive
      ? themeMatches.length
      : slashMatches.length
  ...
  const applySelectedTheme = useCallback(
    (themeId: string) => {
      useThemeStore.getState().setThemePreset(themeId)
      const name = resolveNamedTheme(themeId)?.name ?? themeId
      setInputValue({ text: '', cursorPosition: 0, lastEditDueToNav: false })
      setSlashSelectedIndex(0)
      setMessages((prev) => [
        ...prev,
        getSystemMessage(`Color palette: ${name}`),
      ])
    },
    [setInputValue, setSlashSelectedIndex, setMessages],
  )

  const applySelectedModel = useCallback(
    async (modelId: string) => {
      try {
        const result = await persistModelSelection(modelId)
        useChatStore.getState().setSelectedModel(result.selected)
        setInputValue({ text: '', cursorPosition: 0, lastEditDueToNav: false })
        setSlashSelectedIndex(0)
        setMessages((prev) => [
          ...prev,
          getSystemMessage(formatModelSelectionMessage(result)),
        ])
      } catch (error) {
        setMessages((prev) => [
          ...prev,
          getSystemMessage(
            error instanceof Error ? error.message : `Unknown model "${modelId}"`,
          ),
        ])
      }
    },
  ...
  useEffect(() => {
    ...
    globalThis.addEventListener('Siya:send-followup', handleFollowupClick)
    return () => {
  ...
  const handleSlashItemClick = useCallback(
    async (index: number) => {
      if (isThemePickerActive) {
        const selectedTheme = themeMatches[index]
        if (selectedTheme) {
          applySelectedTheme(selectedTheme.id)
        }
        return
      }

      if (isModelPickerActive) {
        const selectedModel = modelMatches[index]
        if (selectedModel) {
          applySelectedModel(selectedModel.id)
        }
        return
      }
  ...
  const historyNavUpEnabled =
    lastEditDueToNav ||
    (cursorPosition === 0 &&
      ((isSpecialPickerActive && slashSelectedIndex === 0) ||
        (slashContext.active && !isSpecialPickerActive && slashSelectedIndex === 0) ||
        (mentionContext.active && agentSelectedIndex === 0) ||
        (!isSpecialPickerActive && !slashContext.active && !mentionContext.active)))
  const historyNavDownEnabled =
    lastEditDueToNav ||
    (cursorPosition === inputValue.length &&
      ((isSpecialPickerActive &&
        slashSelectedIndex === activeSlashMatchCount - 1) ||
        (slashContext.active &&
          !isSpecialPickerActive &&
  ...
  const chatKeyboardState: ChatKeyboardState = useMemo(
    () => ({
      ...
      feedbackMode,
      focusedAgentId,
      slashMenuActive: slashContext.active || isSpecialPickerActive,
      mentionMenuActive: mentionContext.active,
      slashSelectedIndex,
      agentSelectedIndex,
      slashMatchesLength: activeSlashMatchCount,
  ...
  const chatKeyboardHandlers: ChatKeyboardHandlers = useMemo(
    () => ({
      ...
      onSlashMenuTab: () => {
        const count = activeSlashMatchCount
        if (count <= 1) return
        setSlashSelectedIndex((prev) => (prev + 1) % count)
      },
      onSlashMenuShiftTab: () =>
        setSlashSelectedIndex(
          (prev) => (activeSlashMatchCount + prev - 1) % activeSlashMatchCount,
        ),
      onSlashMenuSelect: async () => {
        if (isThemePickerActive) {
          const selected =
            themeMatches[slashSelectedIndex] || themeMatches[0]
          if (selected) {
            applySelectedTheme(selected.id)
          }
          return
        }
        ...
        if (isModelPickerActive) {
          const selected =
            modelMatches[slashSelectedIndex] || modelMatches[0]
          if (selected) {
            applySelectedModel(selected.id)
          }
          return
        }
  ...
  const hasSlashSuggestions =
    (isModelPickerActive && modelSuggestionItems.length > 0) ||
    (isThemePickerActive && themeSuggestionItems.length > 0) ||
    (slashContext.active &&
      !isSpecialPickerActive &&
      ...
      >
        ...
        {reviewMode ? (
          ...
          <ChatInputBar
            ...
            slashSuggestionItems={activeSlashSuggestionItems}
