...
// Siya downloader extracts both into the same directory. At runtime,
...
if (siblingPath) {
  ...
  process.env.Siya_TREE_SITTER_WASM_PATH = siblingPath
  ...
  ).__Siya_TREE_SITTER_WASM_PATH__ = siblingPath
  ...
  try {
    ...
    ).__Siya_TREE_SITTER_WASM_BINARY__ = new Uint8Array(
      buf.buffer,
      buf.byteOffset,
      buf.byteLength,
    )
