...
import { getSiyaClient } from '../utils/siya-client'
...
import type { AgentDefinition, MessageContent, RunState } from '@siya/sdk'
...
export const useSendMessage = ({
  ...
  const sendMessage = useCallback<SendMessageFn>(
    async ({ content, agentMode, postUserMessage, attachments }) => {
      ...
      if (!client) {
        logger.error({}, '[send-message] Failed to initialize Siya client.')
        setMessages((prev) => [
          ...prev,
          createErrorChatMessage(
            '⚠️ Unable to start the agent. Restart the CLI or check debug logs.',
          ),
        ])
        await yieldToEventLoop()
        setTimeout(() => scrollToLatest(), 0)
