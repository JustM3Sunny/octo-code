...
import {
  ...
  getProjectFileTree,
  type PathInfo,
} from '@siya/common/project-file-tree'
import { SIYA_AVAILABLE_MODELS } from '@siya/common/constants/siya-models'
import { SIYA_NAMED_THEMES } from '../utils/named-themes'
...
/** `/model` or `/model <filter>` — arrow-selectable model list */
const parseModelPickerContext = (input: string): TriggerContext => {
  if (!input) {
    return { active: false, query: '', startIndex: -1 }
  }
  ...
  const [, leadingWhitespace, filterSegment] = match
  const startIndex = lineStart + leadingWhitespace.length
  const query = (filterSegment ?? '').trim().toLowerCase()

  return { active: true, query, startIndex }
}

/** `/theme` or `/theme <filter>` — arrow-selectable palette list */
const parseThemePickerContext = (input: string): TriggerContext => {
  if (!input) {
    return { active: false, query: '', startIndex: -1 }
  }

  const { lineStart, line } = getCurrentLineInfo(input)
  const match = line.match(/^(\s*)\/theme(?:\s+(.*))?$/i)
  if (!match || lineStart !== 0) {
    return { active: false, query: '', startIndex: -1 }
  }
...
export type MatchedModelOption = {
  id: string
  label: string
  description: string
...
const filterModelOptions = (
  ...
  return SIYA_AVAILABLE_MODELS.filter((model) => {
    if (!normalized) return true
    return (
      model.id.toLowerCase().includes(normalized) ||
      model.name.toLowerCase().includes(normalized) ||
      model.apiModelId.toLowerCase().includes(normalized) ||
    ...
    const description = isCurrent
      ? `${model.name} [${model.type}] · ${model.provider} (current)`
      : `${model.name} [${model.type}] · ${model.provider}`
    const firstIndex = normalized ? label.toLowerCase().indexOf(normalized) : -1
    return {
      id: model.id,
      label,
      description,
      ...(firstIndex >= 0 && {
        labelHighlightIndices: createHighlightIndices(
          firstIndex,
          firstIndex + normalized.length,
        ),
      }),
    }
  })
...
export type MatchedThemeOption = {
  id: string
  label: string
  description: string
  labelHighlightIndices?: number[] | null
...
const filterThemeOptions = (query: string): MatchedThemeOption[] => {
  const normalized = query.trim().toLowerCase()
  return SIYA_NAMED_THEMES.filter((theme) => {
    if (!normalized) return true
    return (
...
export interface SuggestionEngineResult {
  ...
  modelPickerContext: TriggerContext
  themePickerContext: TriggerContext
  slashMatches: MatchedSlashCommand[]
  modelMatches: MatchedModelOption[]
  themeMatches: MatchedThemeOption[]
  agentMatches: MatchedAgentInfo[]
  fileMatches: MatchedFileInfo[]
  slashSuggestionItems: SuggestionItem[]
  modelSuggestionItems: SuggestionItem[]
  themeSuggestionItems: SuggestionItem[]
...
export const useSuggestionEngine = ({
  ...
  const modelPickerContext = useMemo(
    () => parseModelPickerContext(deferredInput),
    [deferredInput],
  )

  const themePickerContext = useMemo(
    () => parseThemePickerContext(deferredInput),
    [deferredInput],
  )
  ...
  const modelMatches = useMemo<MatchedModelOption[]>(() => {
    if (!modelPickerContext.active) {
      return []
    }
    return filterModelOptions(modelPickerContext.query, currentSelectedModelId)
  }, [modelPickerContext, currentSelectedModelId])

  const themeMatches = useMemo<MatchedThemeOption[]>(() => {
    if (!themePickerContext.active) {
      return []
    }
    return filterThemeOptions(themePickerContext.query)
  }, [themePickerContext])
  ...
  const slashSuggestionItems = useMemo<SuggestionItem[]>(() => {
    return slashMatches.map((command) => {
      ...
      const modeMatch = command.id.match(/^mode:(default|lite|max|plan)$/i)
      const isCurrentMode =
  ...
  const modelSuggestionItems = useMemo<SuggestionItem[]>(() => {
    return modelMatches.map((model) => ({
      id: model.id,
      label: model.label,
      labelHighlightIndices: model.labelHighlightIndices,
      description: model.description,
      descriptionHighlightIndices: model.descriptionHighlightIndices,
    }))
  }, [modelMatches])

  const themeSuggestionItems = useMemo<SuggestionItem[]>(() => {
    return themeMatches.map((theme) => ({
      id: theme.id,
      label: theme.label,
      labelHighlightIndices: theme.labelHighlightIndices,
      description: theme.description,
      descriptionHighlightIndices: theme.descriptionHighlightIndices,
    }))
  }, [themeMatches])
