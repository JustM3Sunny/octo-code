...
export function getLogoAccentColor(
  ...
  const isTruecolor = supportsTruecolor(env)
  if (themeName === 'dark') {
    return isTruecolor ? '#fab283' : 'yellow'
  }
  return isTruecolor ? '#2968c3' : 'blue'
}
...
const DEFAULT_CHAT_THEMES: Record<ThemeName, ChatTheme> = {
  dark: {
    name: 'dark',
    // OpenCode-inspired semantic colors
    primary: '#fab283',
    secondary: '#5c9cf5',
    success: '#7fd88f',
    error: '#e06c75',
    ...
    // Agent backgrounds
    agentToggleHeaderBg: '#1e1e1e',
    agentToggleExpandedBg: '#282828',
    agentFocusedBg: '#1e1e1e',
    agentContentBg: '#0a0a0a',
    inputFg: '#eeeeee',
