...
/**
 * Strip values that cannot be persisted as JSON (Zod schemas, React render fns, etc.).
 * MCP tool definitions are re-derived on the next run via SDK overrides.
 */
function runStateForPersistence(runState: RunState): RunState {
  const copy = cloneDeep(runState)
  const fileContext = copy.sessionState?.fileContext
  if (fileContext) {
...
function messagesForPersistence(messages: ChatMessage[]): ChatMessage[] {
  return messages.map((message) => {
    const { metadata, blocks, ...rest } = message
    ...
    return {
      ...rest,
      ...(persistedBlocks ? { blocks: persistedBlocks as ContentBlock[] } : {}),
...
export function saveChatState(
  ...
  try {
    ...
    fs.writeFileSync(
      runStatePath,
      JSON.stringify(runStateForPersistence(runState), null, 2),
    )
    fs.writeFileSync(
      messagesPath,
      JSON.stringify(messagesForPersistence(messages), null, 2),
    )
  } catch (error) {
    logger.error(
      {
