...
import { pluralize } from '@siya/common/util/string'
import {
  loadLocalAgents as sdkLoadLocalAgents,
  loadMCPConfigSync,
} from '@siya/sdk'

import type { MCPConfig } from '@siya/common/types/mcp'
...
export interface LocalAgentInfo {
  ...
  displayName: string
  filePath: string
  /** True if this is a bundled Siya agent (not user-created) */
  isBundled?: boolean
}
...
export const loadLocalAgents = (
  ...
  // Get bundled agents - these are the default Siya agents
...
export const loadAgentDefinitions = (): AgentDefinition[] => {
  // Start with bundled agents - these are the default Siya agents
  const bundledAgents = getBundledAgents()
  ...
  const { agentMode, selectedModel } = useChatStore.getState()
  const activeModel = getActiveModelForMode(agentMode, selectedModel)
  return applyModelToAgentDefinitions(definitions, activeModel)
}
