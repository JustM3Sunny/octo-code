...
import { IS_DEV } from '@siya/common/env'
 ...
 * Siya_TRACE=1 in production builds (so users don't pay disk for a second
 * copy of their conversation by default).
 */
export function isTraceEnabled(): boolean {
  if (IS_DEV) return true
  // Same accepted values as Siya_FULL_TELEMETRY (see analytics-sampling)
  const flag = getCliEnv().Siya_TRACE
