...
import { PromptStatusBar } from './prompt-status-bar'
...
export const ChatInputBar = ({
  ...
  const borderColor =
    inputMode === 'default' ? theme.border : theme[modeConfig.color]
        ...
        >
          {modeConfig.label && (
            <box style={{ flexShrink: 0, paddingRight: 1 }}>
              <text>
                <span bg={theme.primary} fg={theme.surface}>{` ${modeConfig.label} `}</span>
              </text>
            </box>
          )}
          {modeConfig.icon && (
    ...
    <>
      ...
      >
        ...
        >
          ...
          {!feedbackMode && inputMode === 'default' ? <PromptStatusBar /> : null}
        </box>
      </box>
      <InputModeBanner />
    </>
