...
export const AgentBranchItem = memo((props: AgentBranchItemProps) => {
  ...
  useWhyDidYouUpdateById('AgentBranchItem', agentId ?? '', props, {
    logLevel: 'debug',
    enabled: getCliEnv().Siya_PERF_TEST === 'true',
  })
  ...
  const toggleFrameColor = isExpanded ? theme.border : theme.muted
  const toggleIconColor = theme.muted
  const toggleIndicator = onToggle ? (isCollapsed ? '▸ ' : '▾ ') : '▸ '
  const toggleLabel = toggleIndicator
  const statusText =
      ...
      >
        ...
        >
          <text style={{ wrapMode: 'none' }}>
            ...
            <span
              fg={isStreaming ? theme.primary : theme.secondary}
