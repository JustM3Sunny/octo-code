...
import {
  formatFullModelHelpForCli,
  getActiveModelForMode,
  getModelDisplayName,
} from '../utils/model-settings'
import { persistModelSelection, formatModelSelectionMessage } from '../utils/model-selection'
...
const ALL_COMMANDS: CommandDefinition[] = [
  ...
  defineCommandWithArgs({
    ...
    handler: async (params, args) => {
      ...
      if (!trimmedArgs) {
        const active = getActiveModelForMode(
          store.agentMode,
          store.selectedModel,
        )
        params.setMessages((prev) => [
          ...
          getSystemMessage(
            `${formatFullModelHelpForCli(active)}`,
          ),
        ])
        params.saveToHistory(params.inputValue.trim())
        clearInput(params)
      ...
      try {
        const result = await persistModelSelection(trimmedArgs)
        store.setSelectedModel(result.selected)
        params.setMessages((prev) => [
          ...prev,
          getUserMessage(params.inputValue.trim()),
        ...
        params.setMessages((prev) => [
          ...
          getUserMessage(params.inputValue.trim()),
          getSystemMessage(
            `Unknown model "${trimmedArgs}". Use /model to list available models.`,
          ),
        ])
      }
      params.saveToHistory(params.inputValue.trim())
  ...
  defineCommandWithArgs({
    ...
    handler: (params, args) => {
      const trimmedArgs = args.trim()
      const { themePresetId, setThemePreset } = useThemeStore.getState()

      if (!trimmedArgs) {
        const lines = [
          'Available palettes (use /theme <id> or type /theme and pick with arrows):',
          '',
          ...SIYA_NAMED_THEMES.map(
            (t) =>
              `  ${t.id}${t.id === themePresetId ? ' (current)' : ''} — ${t.name}: ${t.description}`,
          ),
        ]
        params.setMessages((prev) => [
          ...prev,
      ...
      const resolved = resolveNamedTheme(trimmedArgs)
      if (!resolved) {
        params.setMessages((prev) => [
          ...prev,
          getUserMessage(params.inputValue.trim()),
          getSystemMessage(
            `Unknown palette "${trimmedArgs}". Use /theme to list options.`,
          ),
        ])
        params.saveToHistory(params.inputValue.trim())
        clearInput(params)
      ...
      setThemePreset(resolved.id)
      params.setMessages((prev) => [
        ...prev,
        getUserMessage(params.inputValue.trim()),
        getSystemMessage(`Color palette: ${resolved.name} (${resolved.id})`),
      ])
      params.saveToHistory(params.inputValue.trim())
      clearInput(params)
    },
