...
import type { SiyaToolOutput } from '../../../common/src/tools/list'
 ...
 * Priority:
 * 1. Siya_GIT_BASH_PATH environment variable (user override)
...
function findWindowsBash(env: NodeJS.ProcessEnv): string | null {
  ...
  const customPath = env.Siya_GIT_BASH_PATH
...
2. Use WSL (Windows Subsystem for Linux):
   ...
   Then run Siya inside WSL.

3. Set a custom bash path:
   Set the Siya_GIT_BASH_PATH environment variable to your bash.exe location.
   Example: set Siya_GIT_BASH_PATH=C:\\path\\to\\bash.exe`,
  )
}
...
}): Promise<SiyaToolOutput<'run_terminal_command'>> {
