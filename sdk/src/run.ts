...
import { callMainPrompt } from '@siya/agent-runtime/main-prompt'
import {
  buildUserMessageContent,
  withSystemTags,
} from '@siya/agent-runtime/util/messages'
import { MAX_AGENT_STEPS_DEFAULT } from '@siya/common/constants/agents'
import { toOptionalFile } from '@siya/common/constants/paths'
import {
  getMCPClient,
  listMCPTools,
  callMCPTool,
} from '@siya/common/mcp/client'
import {
  COMPOSIO_META_TOOL_NAMES,
  isComposioMetaToolName,
} from '@siya/common/constants/composio'
import { toolNames } from '@siya/common/tools/constants'
import { clientToolCallSchema } from '@siya/common/tools/list'
import { AgentOutputSchema } from '@siya/common/types/session-state'
import { extractApiErrorDetails } from '@siya/common/util/error'
...
import type { ServerAction } from '@siya/common/actions'
...
import type { PublishedClientToolName } from '@siya/common/tools/list'
...
import type { ToolMessage } from '@siya/common/types/messages/siya-message'
import type {
  ImagePart,
  TextPart,
  ToolResultOutput,
...
import type { SessionState } from '@siya/common/types/session-state'
import type { Source } from '@siya/common/types/source'
import type { SiyaSpawn } from '@siya/common/types/spawn'
...
function isRunPauseError(error: unknown) {
  return (
    !!error &&
    typeof error === 'object' &&
    (('SiyaRunPaused' in error &&
      (error as { SiyaRunPaused?: unknown }).SiyaRunPaused === true) ||
      ('name' in error &&
        (error as { name?: unknown }).name === 'SiyaRunPausedError'))
  )
}

export type SiyaClientOptions = {
  apiKey?: string
  ...
  fsSource?: Source<SiyaFileSystem>
  spawnSource?: Source<SiyaSpawn>
  logger?: Logger
...
export type RunOptions = {
  ...
  signal?: AbortSignal
  costMode?: string
  /** Extra key/values merged into each LLM request's `Siya_metadata`.
...
async function runOnce({
  ...
  if (spawnSource) {
    ...
    spawn = spawnSourceValue as SiyaSpawn
  } else {
    spawn = require('child_process').spawn as SiyaSpawn
  }
...
function requireCwd(cwd: string | undefined, toolName: string): string {
  if (!cwd) {
    throw new Error(
      `cwd is required for the ${toolName} tool. Please provide cwd in SiyaClientOptions or override the ${toolName} tool.`,
    )
  }
  return cwd
}
...
async function readFiles({
  ...
  override?: NonNullable<
    Required<SiyaClientOptions>['overrideTools']['read_files']
  >
  fileFilter?: FileFilter
  cwd?: string
  fs: SiyaFileSystem
...
async function handleToolCall({
  ...
  overrides: NonNullable<SiyaClientOptions['overrideTools']>
  ...
  if (action.type === 'prompt-error') {
    ...
    if (!parsedOutput.success) {
      const message = [
        ...
        'If this issues persists, please contact support@Siya.com',
      ].join('\n')
      onError({ message })
      resolve({
        sessionState: initialSessionState,
