...
import { getSystemInfo } from '@siya/common/util/system-info'
import {
  KNOWLEDGE_FILE_NAMES_LOWERCASE,
  isKnowledgeFile,
} from '@siya/common/constants/knowledge'
import {
  getProjectFileTree,
  getAllFilePaths,
...
async function computeProjectIndex(params: ProjectIndexInput): Promise<{
  ...
  if (filePaths.length > 0) {
    try {
      const { getFileTokenScores } = await import('@siya/code-map/parse')
...
function getFileSize(stats: Awaited<ReturnType<SiyaFileSystem['stat']>>) {
...
export function withMessageHistory({
  ...
  // Deep copy — cloneDeep handles Zod schemas and other non-JSON values in session state
  const newRunState = cloneDeep(runState)
...
export async function applyOverridesToSessionState(
  ...
  // JSON.stringify fails on Zod schemas (e.g. MCP customToolDefinitions) which
  // contain circular references after the first agent run.
