...
describe('Prompt Caching', () => {
  it(
    ...
    async () => {
      if (!apiKey) {
        console.log(
          'Skipping prompt caching integration test: set OPENROUTER_API_KEY to run.\n' +
            'Example: OPENROUTER_API_KEY=your-key bun test src/__tests__/run.integration.test.ts',
        )
        return
      }

      const client = new SiyaClient({ apiKey })
  ...
  it(
    ...
    async () => {
      ...
      try {
        ...
        const client = new SiyaClient({ apiKey, cwd: process.cwd() })
