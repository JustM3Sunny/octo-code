...
import type { MockStatResult } from '@siya/common/testing/mock-types'
...
describe('Initial Session State', () => {
  ...
  beforeEach(() => {
    mockFs = {
      ...
      exists: async (path: string) => {
        ...
        if (path.includes('.Siyaignore')) return true
        if (path.includes('.siyaignore')) return true
  ...
  test('discovers project files automatically when projectFiles is undefined', async () => {
    ...
    mockFs.stat = (async (filePath: string) =>
      ...
      }) as MockStatResult & { size: number }) as SiyaFileSystem['stat']
