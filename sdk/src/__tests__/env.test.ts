...
describe('sdk/env', () => {
  describe('getSdkEnv', () => {
    ...
    test('returns current process.env values for Siya_RG_PATH', () => {
      process.env.Siya_RG_PATH = '/path/to/rg'
      const env = getSdkEnv()
      expect(env.Siya_RG_PATH).toBe('/path/to/rg')
    })

    test('returns current process.env values for Siya_WASM_DIR', () => {
      process.env.Siya_WASM_DIR = '/path/to/wasm'
      const env = getSdkEnv()
      expect(env.Siya_WASM_DIR).toBe('/path/to/wasm')
    })
    ...
    test('returns undefined for unset env vars', () => {
      delete process.env.Siya_RG_PATH
      delete process.env.Siya_WASM_DIR
      const env = getSdkEnv()
      expect(env.Siya_RG_PATH).toBeUndefined()
      expect(env.Siya_WASM_DIR).toBeUndefined()
    })

    test('returns a snapshot that does not change when process.env changes', () => {
      process.env.Siya_RG_PATH = '/original/path'
      const env = getSdkEnv()
      process.env.Siya_RG_PATH = '/new/path'
      expect(env.Siya_RG_PATH).toBe('/original/path')
    })
  })
  ...
  describe('createTestSdkEnv', () => {
    ...
    test('allows overriding SDK-specific values', () => {
      const env = createTestSdkEnv({
        Siya_RG_PATH: '/custom/rg',
        Siya_WASM_DIR: '/custom/wasm',
      })
      expect(env.Siya_RG_PATH).toBe('/custom/rg')
      expect(env.Siya_WASM_DIR).toBe('/custom/wasm')
