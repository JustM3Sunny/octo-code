...
import { promptSuccess } from '@siya/common/util/error'
...
import type { StreamChunk } from '@siya/common/types/contracts/llm'
import type {
  AssistantMessage,
  ToolMessage,
} from '@siya/common/types/messages/Siya-message'
...
describe('tool validation error handling', () => {
  ...
  it('should accept newala typo alias for str_replace replacements', () => {
    const result = parseRawToolCall({
      rawToolCall: {
        toolName: 'str_replace',
        toolCallId: 'newala-typo-tool-call-id',
        input: {
          path: 'test.ts',
          replacements: [
            {
  ...
  it('should treat missing newString as deletion when oldString is present', () => {
    const result = parseRawToolCall({
      rawToolCall: {
        toolName: 'str_replace',
    ...
    if (!('error' in result)) {
      expect(result.input.replacements).toEqual([
        { oldString: 'before', newString: 'after', allowMultiple: false },
        { oldString: 'delete me', newString: '', allowMultiple: false },
        { oldString: 'delete me too', newString: '', allowMultiple: false },
      ])
    }
  })

  it('should summarize missing oldString fields', () => {
    const result = parseRawToolCall({
      rawToolCall: {
        toolName: 'str_replace',
        toolCallId: 'missing-old-tool-call-id',
        input: {
          path: 'test.ts',
          replacements: [
            { oldString: 'before', newString: 'after' },
            { newString: 'orphan new only' },
            { newString: 'another orphan' },
          ],
        },
      },
    })
    ...
    if ('error' in result) {
      ...
      expect(result.error).toContain('- replacements[1].oldString')
      expect(result.error).toContain('- replacements[2].oldString')
