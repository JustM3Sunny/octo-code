import { TEST_USER_ID } from '@siya/common/old-constants'
import { TEST_AGENT_RUNTIME_IMPL } from '@siya/common/testing/impl/agent-runtime'
import { getInitialSessionState } from '@siya/common/types/session-state'
import { assistantMessage } from '@siya/common/util/messages'
import {
  describe,
  expect,
  it,
...
import type { SiyaToolCall } from '@siya/common/tools/list'
...
describe('Spawn Agents Permissions', () => {
  ...
  describe('getMatchingSpawn function', () => {
    describe('exact matches with publisher/agent@version format', () => {
      it('should match exact publisher/agent@version', () => {
        ...
        expect(result).toBe('Siya/thinker@1.0.0')
      })

      it('should not match different versions', () => {
        const spawnableAgents = ['Siya/thinker@1.0.0']
        const result = getMatchingSpawn(
          spawnableAgents,
          'Siya/thinker@2.0.0',
        )
    ...
    describe('publisher/agent format without version', () => {
      it('should match publisher/agent when child has no version', () => {
        const spawnableAgents = ['Siya/thinker@1.0.0', 'acme/reviewer']
        const result = getMatchingSpawn(spawnableAgents, 'Siya/thinker')
        expect(result).toBe('Siya/thinker@1.0.0')
      })

      it('should match exact publisher/agent without version', () => {
        const spawnableAgents = ['Siya/thinker', 'acme/reviewer']
        const result = getMatchingSpawn(spawnableAgents, 'Siya/thinker')
        expect(result).toBe('Siya/thinker')
      })
    ...
    describe('agent@version format without publisher', () => {
      ...
      it('should match agent@version when spawnable has publisher but child does not', () => {
        const spawnableAgents = ['Siya/thinker@1.0.0', 'reviewer@2.0.0']
        const result = getMatchingSpawn(spawnableAgents, 'thinker@1.0.0')
        expect(result).toBe('Siya/thinker@1.0.0')
      })
    ...
    describe('simple agent name format', () => {
      ...
      it('should match simple agent name when spawnable has publisher', () => {
        const spawnableAgents = ['Siya/thinker@1.0.0', 'reviewer']
        const result = getMatchingSpawn(spawnableAgents, 'thinker')
        expect(result).toBe('Siya/thinker@1.0.0')
      })

      it('should match underscored agent name when spawnable has publisher and version', () => {
        const spawnableAgents = ['Siya/file-picker@1.0.0', 'reviewer']
        const result = getMatchingSpawn(spawnableAgents, 'file_picker')
        expect(result).toBe('Siya/file-picker@1.0.0')
      })

      it('should match underscored published agent ID to hyphenated spawnable agent', () => {
        const spawnableAgents = ['Siya/file-picker@1.0.0']
        const result = getMatchingSpawn(
          spawnableAgents,
          'Siya/file_picker@1.0.0',
        )
        expect(result).toBe('Siya/file-picker@1.0.0')
      })
    ...
    describe('edge cases', () => {
      ...
      it('should prioritize exact matches over partial matches', () => {
        const spawnableAgents = ['thinker', 'Siya/thinker@1.0.0']
        const result = getMatchingSpawn(spawnableAgents, 'thinker')
  ...
  describe('handleSpawnAgents permission validation', () => {
    ...
    it('should allow underscored published agent_type when hyphenated agent is spawnable', async () => {
      const parentAgent = createMockAgent('parent', [
        'Siya/file-picker@1.0.0',
      ])
      const childAgent = createMockAgent('Siya/file-picker@1.0.0')
      const sessionState = getInitialSessionState(mockFileContext)
      const toolCall = createSpawnToolCall('Siya/file_picker@1.0.0')

      const { output } = await handleSpawnAgents({
        ...handleSpawnAgentsBaseParams,
        agentState: sessionState.mainAgentState,
        agentTemplate: parentAgent,
        localAgentTemplates: { 'Siya/file-picker@1.0.0': childAgent },
        toolCall,
      })
    ...
    it('should handle versioned agent permissions correctly', async () => {
      const parentAgent = createMockAgent('parent', ['Siya/thinker@1.0.0'])
      const childAgent = createMockAgent('Siya/thinker@1.0.0')
      const sessionState = getInitialSessionState(mockFileContext)
      const toolCall = createSpawnToolCall('Siya/thinker@1.0.0')

      const { output } = await handleSpawnAgents({
        ...handleSpawnAgentsBaseParams,
        agentState: sessionState.mainAgentState,
        agentTemplate: parentAgent,
        localAgentTemplates: { 'Siya/thinker@1.0.0': childAgent },
        toolCall,
      })
    ...
    it('should allow spawning simple agent name when parent allows versioned agent', async () => {
      ...
      const { output } = await handleSpawnAgents({
        ...
        localAgentTemplates: {
          thinker: childAgent,
          'Siya/thinker@1.0.0': childAgent, // Register with both keys
        },
        toolCall,
      })
    ...
    it('should reject when version mismatch exists', async () => {
      const parentAgent = createMockAgent('parent', ['Siya/thinker@1.0.0'])
      const childAgent = createMockAgent('Siya/thinker@2.0.0')
      const sessionState = getInitialSessionState(mockFileContext)
      const toolCall = createSpawnToolCall('Siya/thinker@2.0.0')

      const { output } = await handleSpawnAgents({
        ...handleSpawnAgentsBaseParams,
        agentState: sessionState.mainAgentState,
        agentTemplate: parentAgent,
        localAgentTemplates: { 'Siya/thinker@2.0.0': childAgent },
        toolCall,
      })
    ...
    it('should handle multiple agents with mixed success/failure', async () => {
      ...
      const toolCall: SiyaToolCall<'spawn_agents'> = {
        toolName: 'spawn_agents' as const,
        toolCallId: 'test-tool-call-id',
        input: {
          agents: [
  ...
  describe('handleSpawnAgentInline permission validation', () => {
    const createInlineSpawnToolCall = (
      agentType: string,
      prompt = 'test prompt',
    ): SiyaToolCall<'spawn_agent_inline'> => ({
    ...
    it('should handle versioned inline agent permissions correctly', async () => {
      ...
      const childAgent = createMockAgent('Siya/thinker@1.0.0')
      const sessionState = getInitialSessionState(mockFileContext)
      const toolCall = createInlineSpawnToolCall('Siya/thinker@1.0.0')
    ...
    it('should reject inline spawn when version mismatch exists', async () => {
      ...
      const childAgent = createMockAgent('Siya/thinker@2.0.0')
      const sessionState = getInitialSessionState(mockFileContext)
      const toolCall = createInlineSpawnToolCall('Siya/thinker@2.0.0')
