...
function simplifyTerminalHelper(params: {
  toolResult: SiyaToolOutput<'run_terminal_command'>
  numKept: number
  logger: Logger
}): { result: SiyaToolOutput<'run_terminal_command'>; numKept: number } {
  ...
  for (let i = messages.length - 1; i >= 0; i--) {
    ...
    if (m.role === 'system' || m.role === 'user' || m.role === 'assistant') {
      ...
      const terminalResultMessage = cloneDeep(
        m,
      ) as SiyaToolMessage<'run_terminal_command'>
...
export function getMessagesSubset(params: {
  ...
  for (const message of messagesSubset) {
    for (const provider of ['anthropic', 'openrouter', 'Siya'] as const) {
  ...
  for (const message of messages) {
    ...
    if (message.toolName === 'read_files') {
      try {
        files.push(
          ...(
            message as SiyaToolMessage<'read_files'>
    ...
    if (message.toolName === 'find_files') {
      try {
        const v = (message as SiyaToolMessage<'find_files'>).content[0]
          .value
        if ('message' in v) {
          continue
        }
