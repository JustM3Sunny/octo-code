import { endsAgentStepParam, toolNames } from '@siya/common/tools/constants'
import { toolParams } from '@siya/common/tools/list'
import { generateCompactId } from '@siya/common/util/string'
...
import type {
  ...
  AgentState,
  Subgoal,
} from '@siya/common/types/session-state'
import type {
  CustomToolDefinitions,
  ProjectFileContext,
} from '@siya/common/util/file'
...
export type ExecuteToolCallParams<T extends string = ToolName> = {
  ...
  toolCalls: (SiyaToolCall | CustomToolCall)[]
  toolCallsToAddToMessageHistory: (SiyaToolCall | CustomToolCall)[]
...
export async function executeToolCall<T extends ToolName>(
  ...
  const toolCall: SiyaToolCall<T> | ToolCallError = parseRawToolCall<T>({
    rawToolCall: {
      toolName,
      toolCallId,
      input,
  ...
  const handler = SiyaToolHandlers[
    toolName
  ] as unknown as SiyaToolHandlerFunction<T>
  ...
  const toolResultPromise = handler({
    ...
    requestClientToolCall: (async (
      ...
      return clientToolResult.output as SiyaToolOutput<T>
    }) as any,
  })
...
export function parseRawCustomToolCall(params: {
  ...
  // Add the required Siya_end_step parameter with the correct value for this tool if requested
