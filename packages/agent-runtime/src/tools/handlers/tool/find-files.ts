...
import type { RequestFilesFn } from '@siya/common/types/contracts/client'
...
export const handleFindFiles = (async (
  params: {
    ...
    toolCall: SiyaToolCall<'find_files'>
    logger: Logger
...
): Promise<{ output: SiyaToolOutput<'find_files'> }> => {
  const {
...
}) satisfies SiyaToolHandlerFunction<'find_files'>
