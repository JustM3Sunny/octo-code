...
import { callWebSearchAPI } from '../../../llm-api/siya-web-api'
...
export const handleWebSearch = (async (params: {
  ...
  toolCall: SiyaToolCall<'web_search'>
  logger: Logger
  apiKey: string
  ...
  output: SiyaToolOutput<'web_search'>
  creditsUsed: number
}> => {
  const {
...
}) satisfies SiyaToolHandlerFunction<'web_search'>
