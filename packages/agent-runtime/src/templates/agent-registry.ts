import { validateAgents } from '@siya/common/templates/agent-validation'
import {
  normalizeAgentIdForLookup,
  parsePublishedAgentId,
} from '@siya/common/util/agent-id-parsing'
import { DEFAULT_ORG_PREFIX } from '@siya/common/util/agent-name-normalization'

import type { DynamicAgentValidationError } from '@siya/common/templates/agent-validation'
import type { AgentTemplate } from '@siya/common/types/agent-template'
import type { FetchAgentFromDatabaseFn } from '@siya/common/types/contracts/database'
...
export async function getAgentTemplate(
  ...
  if (!parsed) {
    // If agentId doesn't parse as publisher/agent format, try as Siya/agentId
    const SiyaParsed = parsePublishedAgentId(
      `${DEFAULT_ORG_PREFIX}${normalizedAgentId}`,
    )
    if (SiyaParsed) {
      const dbAgent = await fetchAgentFromDatabase({
        ...params,
        parsedAgentId: SiyaParsed,
      })
      if (dbAgent) {
