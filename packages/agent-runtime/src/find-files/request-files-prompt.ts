...
} from '@siya/common/old-constants'
import { getAllFilePaths } from '@siya/common/project-file-tree'
import { isAbortError, unwrapPromptResult } from '@siya/common/util/error'
import { systemMessage, userMessage } from '@siya/common/util/messages'
...
import type { PromptAiSdkFn } from '@siya/common/types/contracts/llm'
...
async function promptFlashWithFallbacks(
  params: {
    messages: Message[]
  ...
  const { messages, useFinetunedModel, promptAiSdk, logger } = params
  ...
  try {
    ...
    logger.warn(
      { error },
      'Error calling Gemini API, falling back to Claude',
    )
    return unwrapPromptResult(
      await promptAiSdk({
        ...params,
        messages,
        model: openrouterModels.openrouter_claude_3_5_haiku,
      }),
    )
  }
}
...
async function getRelevantFiles(
  ...
  let SiyaMessages = [systemMessage(system), ...messagesWithPrompt]

  // Converts assistant messages to user messages for finetuned model
  SiyaMessages = SiyaMessages
    .map((msg, i) => {
      if (msg.role === 'assistant' && i !== SiyaMessages.length - 1) {
