...
const TREE_SITTER_WASM_ENV_VAR = 'Siya_TREE_SITTER_WASM_PATH'
const WASM_BINARY_GLOBAL_KEY = '__Siya_TREE_SITTER_WASM_BINARY__'
...
// (freebuff) / pre-1.0.666 (Siya) wrappers download the new binary
 ...
 * bytes onto `globalThis.__Siya_TREE_SITTER_WASM_BINARY__` because Windows
...
function resolveTreeSitterWasm(scriptDir: string): string {
  ...
  // tree-sitter.wasm alongside `freebuff.exe` / `Siya.exe` because
