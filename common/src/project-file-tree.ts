...
import type { SiyaFileSystem } from './types/filesystem'
...
function logFileTreeError(
  ...
  if (!process.env.DEBUG && !process.env.Siya_DEBUG) {
    return
  }
...
export async function parseGitignore(params: {
  ...
  const ignoreFiles = [
    ...
    path.join(fullDirPath, '.Siyaignore'),
    path.join(fullDirPath, '.siyaignore'), // Legacy support
  ]
