...
import type { ToolMessage } from '../types/messages/siya-message'
...
export type SiyaToolCall<T extends ToolName = ToolName> = {
  [K in ToolName]: {
    toolName: K
...
export type SiyaToolOutput<T extends ToolName = ToolName> = {
  [K in ToolName]: K extends ToolName
    ? z.infer<(typeof toolParams)[K]['outputSchema']>
    : never
}[T]

export type SiyaToolMessage<T extends ToolName = ToolName> = ToolMessage & {
  content: SiyaToolOutput<T>
}
