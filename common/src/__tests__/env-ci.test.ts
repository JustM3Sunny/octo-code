...
describe('env-ci', () => {
  describe('getCiEnv', () => {
    ...
    test('returns current process.env values for OPENROUTER_API_KEY', () => {
      process.env.OPENROUTER_API_KEY = 'test-key-123'
      const env = getCiEnv()
      expect(env.OPENROUTER_API_KEY).toBe('test-key-123')
    })

    test('returns current process.env values for SIYA_GITHUB_TOKEN', () => {
      process.env.SIYA_GITHUB_TOKEN = 'ghp_test_token'
      const env = getCiEnv()
      expect(env.SIYA_GITHUB_TOKEN).toBe('ghp_test_token')
    })
  ...
  describe('ciEnv', () => {
    ...
    test('contains expected keys', () => {
      ...
      expect('OPENROUTER_API_KEY' in ciEnv).toBe(true)
      expect('SIYA_GITHUB_TOKEN' in ciEnv).toBe(true)
    })
  })
  ...
  describe('createTestCiEnv', () => {
    ...
    test('returns undefined for most vars by default', () => {
      ...
      expect(env.SIYA_GITHUB_TOKEN).toBeUndefined()
    })
    ...
    test('allows overriding default values', () => {
      ...
      expect(env.OPENROUTER_API_KEY).toBe('custom-api-key')
    })
    ...
    test('allows setting all CI-related vars for CI simulation', () => {
      const env = createTestCiEnv({
        ...
        SIYA_GITHUB_TOKEN: 'ghp_simulated_token',
        OPENROUTER_API_KEY: 'test-api-key-override',
      })
      expect(env.CI).toBe('true')
      expect(env.GITHUB_ACTIONS).toBe('true')
      expect(env.RENDER).toBe('true')
      expect(env.IS_PULL_REQUEST).toBe('true')
      expect(env.SIYA_GITHUB_TOKEN).toBe('ghp_simulated_token')
      expect(env.OPENROUTER_API_KEY).toBe('test-api-key-override')
    })
  })
})
