...
describe('env-process', () => {
  describe('getProcessEnv', () => {
    ...
    test('returns current process.env values for Siya_IS_BINARY', () => {
      process.env.Siya_IS_BINARY = 'true'
      const env = getProcessEnv()
      expect(env.Siya_IS_BINARY).toBe('true')
    })
  ...
  describe('createTestProcessEnv', () => {
    ...
    test('returns undefined for most vars by default', () => {
      ...
      expect(env.KITTY_WINDOW_ID).toBeUndefined()
      expect(env.Siya_IS_BINARY).toBeUndefined()
    })
    ...
    test('allows setting Siya_IS_BINARY for binary mode tests', () => {
      const env = createTestProcessEnv({
        Siya_IS_BINARY: 'true',
        Siya_CLI_VERSION: '1.0.0',
      })
      expect(env.Siya_IS_BINARY).toBe('true')
      expect(env.Siya_CLI_VERSION).toBe('1.0.0')
    })
